...
gt_var_sum <- function(statistic) {
    ...
    eve_tbl_summary %>% 
        ...
        group_by(g_PLS) %>% 
        gt::gt() %>% 
            tab_header(title = glue("The {statistic} of explanation variables"),
                       subtitle = glue("quarter {(Accprd + days(1)) %m+% months(-3)} to {Accprd}")
                       ) %>% 
...
lm_trd <- function(df, model_type, SE_type = "stata", ...) {
    ...
    if (model_type == "agg") {  # running all data at once (Cluster-Robust Variance)
            lm_result <- lm_robust(data = df, clusters = g_PLS, se_type = SE_type, ...,
                    formula = AbRet ~ Amplitude + Turnover + Liquidility + Dnshrtrd)
    } else if (model_type == "lst") {  # running separate by group (list-column)
            lm_result <- lm_robust(data = df, se_type = SE_type, ...,
                    formula = AbRet ~ Amplitude + Turnover + Liquidility + Dnshrtrd)
    } else if (model_type == "inter") {  # take the group information as the interaction
            lm_result <- lm_robust(data = df, se_type = SE_type, ...,
...
if (is.numeric(subsam)) {  # take a subset of stocks sample
        ...
        print("Analysis the whole sample.")
...
# to speed up the procession, we take a restrict on trading date
qtr_term <- seq(as.numeric(Accprd %m+% months(-9)), 
                as.numeric(Accprd %m+% months(+9)), 
                by = 1
                )
# link to the SQLite database
QEA_db <- dbConnect(SQLite(), "~/OneDrive/Data.backup/QEAData/QEA_db.sqlite")
...
if (!all.equal(names(stkeve), names(trddat))) {
        ...
        if (!all.equal(rep(TS, N), unname(sapply(eve_tbl, nrow)))) {
                ...
                print(glue("{extra_stk}, unusual! it's number of time periods not equal to {TS}."))
...
mod_figure("Dret_sd", fig_ratio = 3L, scale = 0.8)
...
g_AMD <- select(ReptInfo, c("Stkcd", "Accper", all_of(g_Acc))) %>% 
        spread(Accper, g_Acc) %>% 
...
title_char <- paste("The average daily returns within event window of portfolios",
        "\nwhich are structured by the difference of EPS among two quarters",
        sep = " "
        )
caption_char <- glue("Grouped by {g_Acc}, {(Accprd + days(1)) %m+% months(-3)} to {Accprd}")
    ...
    filter(g.AMD != "Neutral") %>% 
        ggplot(aes(x = as.integer(as.character(Timeline)), 
            ...
            labs(y = "Average daily return", x = "Time line",
                 title = title_char, caption = caption_char
                 ) +
...
mod_figure("Diff", fig_ratio = 3L, scale = 0.6)
...
# generate regression results table ====
    ## trading explanation
    list(lm_trd(eve_tbl, model_type = "agg"),  # aggregate
    ...
    list(lm_ff(eve_tbl, ts_type = "t", model_type = "agg"),  
    ...
    inner_join(AR_tau, AMD_tau, by = "Timeline") %>% 
            group_by(g_PLS) %>% nest() %>% 
            transmute("ts_ff_tau" = map(data, lm_ff, ts_type = "tau", model_type = "lst")) %>%
            pluck("ts_ff_tau") %>% 
            list(., 
                 lm_ff(df = inner_join(AR_tau, AMD_tau, by = "Timeline"), 
                       ts_type = "tau", 
                       model_type = "inter"
                       )
                 ) %>% 
...
# model visualization -----------------------------------------------------
    ...
    plot_tau_inter <- function(df, str_grep, ylim1 = "estimate", ylim2 = "statistic"
                               ) {
...
cs_tlm_tau_inter <- plot_tau_inter(cs_tlm, str_grep = "^Timeline([:^:]\\d+|\\.[LQC])")
