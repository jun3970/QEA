...
Accprd %<>% as.Date(origin = '1970-01-01') 
...
if (model_type %in% c("FF3", "FF4", "FF5", "CH3", "CH4")) {
...
for (q in seq_along(Accprd)) {  # loop in time period
    ...
    trdwin <- trddat %>%  
            mutate(data = map(data, filter, TradingDate %within% interval(base_date, Accprd[q]))) %>% 
            unnest(cols = "data") %>% 
            split(.$Stkcd)
    ...
    if (period_type == "quarterly") {  # less than 15 trading records in the past month
            ...
            trdwin %<>% keep(~ filter(.x, TradingDate %within% interval(Accprd[q] %m+% months(-1), Accprd[q])) %>% 
                                nrow() >= 15L
                             )
        
    } else if (period_type == "yearly") {  # less than 180 trading records in the past year
        
            trdwin %<>% keep(~ filter(.x, TradingDate %within% interval(Accprd[q] + years(-1), Accprd[q])) %>% 
                                nrow() >= 180L
                             )
    }
    ...
    if (model_type %in% c("CH3", "CH4", "FF3", "FF4", "FF5")) {
            ...
            if (model_type == "CH3") {
                    ...
                    if (value_base == "EPS") {
                            ...
                            ReptInfo_Acc_Income <- tbl_query(table_name = "Income_Statement") %>% 
                                    ...
                                    collect() %>% 
                                    rename("Earning" = B002000000, "EPS" = B003000000)
                            ...
                            ReptInfo_Acc_Income <- inner_join(ReptInfo_Acc_Earning, ReptInfo_Acc_CashFlow,
                                                              by = c("Stkcd", "Accper"))
                    }
                    ...
                    ReptInfo_Acc <- inner_join(ReptInfo_Acc_Income, ReptInfo_Acc_Balance,
                                               by = c("Stkcd", "Accper"))
            }
        ...
        if (model_type %in% c("FF3", "FF4", "FF5")) {
            ...
            if (model_type == "FF3") {
                    ...
                    ReptInfo_Acc <- tbl_query(table_name = "Balance_Sheet") %>% 
                            ...
                            collect() %>% 
                            rename("Asset" = A001000000, "Liability" = A002000000) %>% 
                    ...
                    ReptInfo_Acc_Balance <- tbl_query(table_name = "Balance_Sheet") %>% 
                            ...
                            collect() %>% 
                            rename("Asset" = A001000000, "Liability" = A002000000, "Equity" = A004000000) %>% 
                            mutate("Asset_Net" = Asset - Liability) %>% 
                    ...
                    ReptInfo_Acc_Asset <- tbl_query(table_name = "Balance_Sheet", date = link_period) %>% 
                    ...
                    ReptInfo_Acc <- inner_join(ReptInfo_Acc_Income, ReptInfo_Acc_Balance, by = c("Stkcd")) %>%
                            inner_join(ReptInfo_Acc_Asset, by = c("Stkcd")) 
                
            }
        }        
        
        ReptInfo_Acc %<>% mutate('Accper' = as.Date(Accper, origin = "1970-01-01")) %>% 
        ...
        potfol <- trdwin %>% 
                ...
                map_dfr(filter, TradingDate == ymd(base_date)) %>% 
                ...
                # join the data of stock's share number
        ...
        if (model_type %in% c("CH3", "CH4")) {
                ...
                if(model_type == "CH4")  # profitability, earnings-to-book equity (ROE)
        ...
        if (model_type %in% c("FF3", "FF4", "FF5")) {
                ...
                potfol %<>% mutate('Value' = Asset_Net / Size)  # Value, Book value to Market value
                ...
                if (model_type == "FF4") {
                        ...
                        potfol %<>% mutate('Profitability' = Earning / Equity) %>% 
                                rename("Investment" = Asset_Growth_Rate)  # Investment, total asset growth rate
                }
        }
    ...
    trdff %<>% map_dfr(~ right_join(Nrrate, select(.x, c(Stkcd, TradingDate, Dretnd, Dsmvosd)), 
                                    by = 'TradingDate')
                       ) %>% 
    ...
    if (model_type %in% c("CH3", "CH4", "FF3", "FF4", "FF5")) {
            ...
            if(model_type %in% c("CH3", "FF3")) {
                    ...
                    trdff_factor %<>% mutate(
                            "Portfolio_Return" = map(data, summarise, "Ret" = Dretnd %*% (Dsmvosd / sum(Dsmvosd)))
                            )
                    # calculate the values of factors
                    trdff_factor %<>% mutate(
                            "SMB" = map_dbl(Portfolio_Return, 
                                            ~ summarise(group_by(.x, g_Size), "avgRet" = mean(Ret)) %>% 
                                              spread(key = g_Size, value = avgRet) %>% 
                                              with(., Small - Big)
                                            ),
                            "VMG" = map_dbl(Portfolio_Return, 
                                            ~ summarise(group_by(.x, g_Value), "avgRet" = mean(Ret)) %>% 
                                              spread(key = g_Value, value = avgRet) %>% 
                                              with(., Value - Growth)
                                            )
                            )
                    ...
                    trdff_factor %<>% mutate(
                            ...
                            "RMW" = map_dbl(SP_Ret, 
                                            ~ summarise(group_by(.x, g_Profitability), "avgRet" = mean(Ret)) %>% 
                                              spread(key = g_Profitability, value = avgRet) %>% 
                                              with(., Robust - Weak)
                                            ),
                            # SMB
                    ...
                    trdff_factor %<>% mutate(
                            ...
                            "CMA" = map_dbl(SI_Ret, 
                                            ~ summarise(group_by(.x, g_Investment), "avgRet" = mean(Ret)) %>% 
    ...
    if (model_type %in% c("CH3", "CH4", "FF3", "FF4", "FF5")) {
            ...
            portfolio_ret <- trdff %>% 
                    transmute(TradingDate, 
                              "PR" = map(data, 
                                         ~ inner_join(.x, select(potfol_y, Stkcd, starts_with('g_')), 
                                                      by = "Stkcd") %$% 
                                           summarise(group_by_if(., is.factor), 
            ...
            if (model_type %in% c("CH3", "CH4", "FF3")) {  # plot line figure
                    ...
                    if (model_type %in% c("CH3", "CH4")) {
                            ...
                            figure_filename <- paste(Accprd[q], period_type, value_base, model_type,
                                                     "portfolio_return.pdf", sep = "_")
                    
                    } else  figure_filename <- paste(Accprd[q], period_type, model_type,
                                                     "portfolio_return.pdf", sep = "_")
                    ...
                    if (model_type %in% c("CH3", "FF3")) { 
                            ...
                            figure_ptf_Ret <- portfolio_ret %>% 
                                    filter(g_Size %in% c("Small", "Big"), 
                            ...
                            title_char <- paste("Daily return of portfolios which structured in size", 
                                                "value, and profitability", model_type, sep = ", ")
                            
                            figure_ptf_Ret <- portfolio_ret %>% 
                                    filter(g_Size %in% c("Small", "Big"),
                                           g_Value %in% c("Value", "Growth"), 
                                           g_Profitability %in% c("Robust", "Weak")
                                           ) %>% 
                                    ggplot(mapping = aes(x = TradingDate, y = ptf_Ret)) +
                                    geom_line(aes(colour = g_Profitability, linetype = g_Value)) + 
                                    facet_wrap(~ g_Size, ncol = 2)
                    }
                    
                    figure_ptf_Ret + 
                    labs(y = "Weighted daily return", title = title_char) +
    ...
    # Running Regression ---------------------------------------------------
            ...
            potfolreg[[q]] <- inner_join(portfolio_ret, Nrrate, by = "TradingDate") %>%  
                    inner_join(select(trdff_factor, TradingDate, mkt_rf, all_of(ff_term)), 
                               by = "TradingDate") %>% 
                    group_by_if(is.factor) %>% nest() %>%  
    ...
    } else if (model_type == "CAPM") {
        
            potfolreg[[q]] <- inner_join(select(trdff, TradingDate, mkt_rf), Nrrate, by = "TradingDate") %>% 
                    lapply(trdwin, inner_join, ., by = "TradingDate") %>% 
                    map(~ ifelse(nrow(.x) <= 40L, return(NULL), return(.x))) %>% compact() %>% 
                    map_dfr(~ tidy(lm(formula = model_formula, data = .x)), .id = "Stkcd")
    } 
}
...
if (model_type == "CAPM") { # the distribution of the estimator of beta (CAPM) ====
    ...
    map_dfr(potfolreg, subset, term == 'mkt_rf', select = 'estimate', .id = 'quarter') %>% 
    filter(quarter >= ymd('2013-03-31')) %>% 
    ggplot(data = ., mapping = aes(x = estimate, y=..density..)) +
        geom_histogram(bins = 20) + geom_density() + 
        facet_wrap(facets = vars(quarter), ncol = 4) + 
            labs(title = "The distribution of the estimated beta (CAPM)",
                 x = "The value of estimate beta of stocks") + 
            scale_x_continuous(limits = c(-0.5, 2.5), n.breaks = 6) 

    ggsave(filename = glue('./{model_type}/{model_type}_beta_hist.pdf'),
           width = 18, height = 18)

} else if (model_type %in% c("CH3", "CH4", "FF4", "FF5")) {  
    # visualize the values of factors by month ====
    Accprd_year <- unique(year(Accprd))
    FF_factor <- map_dfr(potfolreg, ~ select(.x$data[[1]], 'TradingDate', all_of(ff_term))) %>% 
            gather(key = Factor, value = Return, -TradingDate) %>% 
            mutate("month" = month(TradingDate, label = TRUE),  # facet
                   "day" = day(TradingDate),  # x axis
                   Factor = factor(Factor, levels = ff_term))
    
    for (i in seq_along(Accprd_year)) {
        
        ggplot(data = filter(FF_factor, year(TradingDate) == Accprd_year[i])) + 
            # The months that earnings report are released
            geom_rect(data = data.frame("month" = factor(month.abb[-c(3,4,8,10)])),
            ...
            geom_path(aes(x = day, y = Return, linetype = Factor, color = Factor)) + 
                facet_wrap( ~ month, ncol = 3) +
                    labs(y = "The value of Mimicking-risk factor", x = "Calendar day",
                         title = "The first difference value of diversified portfolios") +
                    scale_color_brewer(palette = "Set1") + 
                    theme_replace(legend.title = element_text(inherit.blank = FALSE))

        ifelse(model_type %in% c("CH3", "CH4"),
               figure_filename <- paste(Accprd_year[i], value_base, model_type, 'factor_path.pdf', sep = '_'),
               figure_filename <- paste(Accprd_year[i], model_type, 'factor_path.pdf', sep = '_')
               )
        
        ggsave(filename = file.path(model_type, figure_filename),
